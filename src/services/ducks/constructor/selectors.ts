...
export const getBreadArray = createSelector(getData, dataArray => dataArray.filter((el: Ingredient) => el.type === BUN))
export const getFillingArray = createSelector(getData, dataArray => dataArray.filter((el: Ingredient) => el.type === MAIN))
export const getSauceArray = createSelector(getData, dataArray => dataArray.filter((el: Ingredient) => el.type === SAUCE))
export const getSendOrderArray = createSelector(getOrderData, getBread, (orderData, bread) => [...orderData, bread])

export const getProductArray = createSelector(getOrderData, items => items.filter((el: Ingredient) => el.type !== BUN))
